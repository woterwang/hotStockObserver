...
export class VolumeSurgeService {
  ...
  private async fetchFromWencai (dateStr: string): Promise<any[]> {
    ...
    const question = [
      // 核心条件（宽松版，确保有数据）
      // `${dateStr}涨幅>7%`,
      //20260410涨幅>8%，收盘价创120日新高，20260410成交量，20260410开盘价，20260410收盘价，20260410上引线，20260410量比，所属概念，流通市值，上市>100天，非ST，非退市，非北交所，近二年未被立案
      `${dateStr}涨幅>8%`,
      `${dateStr}收盘价创120日新高`,
  ...
  async scanAndSave (dateStr?: string): Promise<number> {
    ...
    logger.info(`开始扫描放量大涨新高股票: ${targetDate}`);
    ...
    for (const item of rawData) {
      try {
        ...
        // 动态查找字段
        for (const key in item) {
          if (key.includes(`开盘价:不复权[${targetDate}]`)) {
            openPrice = parseFloat(item[key] || 0);
          }
          if (key.includes(`收盘价:不复权[${targetDate}]`)) {
            closePrice = parseFloat(item[key] || 0);
          }
          if (key.includes('股市值')) {
            marketCapitalization = parseFloat(item[key] || 0);
          }
          if (key.includes(`上市天数[${targetDate}]`)) {
