...
export class TrendScorer {
  ...
  static calculate (kline: KLineItem[]): { score: number; tag: AnalysisOutput['trendTag'] } {
    //打印第一条K线与最后一条K线的日期，验证数据顺序和完整性
    logger.info(`[TrendScorer] 第一天开始：${kline[0].date} - ${kline[kline.length - 1].date}`);
    ...
    // const change5Series = this.buildChangeSeries(kline, 5, this.ADAPTIVE_LOOKBACK);
    // const change10Series = this.buildChangeSeries(kline, 10, this.ADAPTIVE_LOOKBACK);
    ...
    // const slope5Series = this.buildSlopeSeries(kline, 5, this.ADAPTIVE_LOOKBACK);
    // const slope10Series = this.buildSlopeSeries(kline, 10, this.ADAPTIVE_LOOKBACK);
    // const slope20Series = this.buildSlopeSeries(kline, 20, this.ADAPTIVE_LOOKBACK);
    ...
    //   const divergenceSeries = this.buildMADivergenceSeries(kline, 5, 20, this.ADAPTIVE_LOOKBACK);
    ...
    // const volRatioSeries = this.buildVolRatioSeries(kline, this.ADAPTIVE_LOOKBACK);
