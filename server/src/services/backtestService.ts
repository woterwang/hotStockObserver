...
import { TradingSignal,PriceBreakthrough } from '../models';
...
export class BacktestService {
  ...
  async runBacktest (
    ...
    const breakthroughRecords = await TradingSignal.find({
      signalDate: { $gte: startDate, $lte: endDate },
      status: 'ready',
    }).sort({ date: 1 }).lean();
    ...
    for (const record of breakthroughRecords) {
      ...
      const day3Date = record.signalDate as string;
